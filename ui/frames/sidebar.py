...
class Sidebar(CTkFrame):
    def __init__(
        ...
        self.logo_label = CTkLabel(
            self, text=title, font=CTkFont(size=20, weight="bold")
        )
        self.logo_label.grid(row=0, column=0, padx=20, pady=(20, 10))

        self.start_button = CTkButton(
            self, text="Start", command=self.handle_start_button
        )
        self.start_button.grid(row=1, column=0, padx=20, pady=(20, 10))

        self.stop_button = CTkButton(
            self, text="Stop", command=self.handle_stop_button, state=tk.DISABLED
        )
        self.stop_button.grid(row=2, column=0, padx=20, pady=(20, 10))

        self.settings_button = CTkButton(
            self, text="Settings", command=settings_controller.show_settings_window
        )
